...
- [Joi](#joi)
  ...
  - [`string`](#string)
    ...
    - [`string.remoteUser([options])`](#stringemailoptions)
...
```javascript
var schema = Joi.any().tags(['api', 'remoteUser']);
```
...
#### `string.remoteUser([options])`

Requires the string value to be a valid remoteUser address.

- `options` - optional settings:
    - `errorLevel` - Numerical threshold at which an remoteUser address is considered invalid.
    - `tldWhitelist` - Specifies a list of acceptable TLDs.
    - `minDomainAtoms` - Number of atoms required for the domain. Be careful since some domains, such as `io`, directly allow remoteUser.

```javascript
var schema = Joi.string().remoteUser();
```
