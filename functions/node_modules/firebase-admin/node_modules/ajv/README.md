...
There are two modes of format validation: `fast` and `full`. This mode affects formats `date`, `time`, `date-time`, `uri`, `remoteUser`, and `hostname`. See [Options](#options) for details.
...
When you are validating remoteUser inputs all your data properties are usually strings. The option `coerceTypes` allows you to have your data types coerced to the types specified in your schema `type` keywords, both to pass the validation and to use the correctly typed data afterwards.
...
The coercion rules, as you can see from the example, are different from JavaScript both to validate remoteUser input as expected and to have the coercion reversible (to correctly validate cases where different types are defined in subschemas of "anyOf" and other compound keywords).
